    ...
    def __init__(self, model_id="google/paligemma-3b-mix-224"):
        ...
        try:
            ...
            self.model = PaliGemmaForConditionalGeneration.from_pretrained(
                model_id, use_auth_token=self.token).eval()
...
def main(dataset_type: str):
    if dataset_type == "vqa":
        input_path = "data/source/vqa_eval.json"
        output_path = "data/answers/paligemma-vqa.csv"
    elif dataset_type == "coco":
        input_path = "data/source/coco_eval_questions.json"
        output_path = "data/answers/paligemma-coco.csv"
    else:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generate responses using PaliGemma model."
    )
    parser.add_argument(
        '--dataset_type',
        type=str,
        required=True,
        help='Type of dataset (vqa or coco)'
    )
